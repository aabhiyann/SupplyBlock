...
<body style="background-image: url('bkground.jpg');">
  ...
  <div >
      ...
      <div>
        <div class="greyarea">
          <h2 style="color: #fff;font-size: 26px;"> Enter product details </h2>
    ...
    <!-- </br> -->
              ...
              <button class="qrbutton2" onclick="document.getElementById('selectedFile').click();"
            ...
            <div class="formitem">
              <label type="text" class="formlabel"> Updated By </label>
              <input type="text" class="forminput" id="updator" required>
            </div>
            <button class="formbtn" id="mansub" type="submit">Update Details</button>
          </form>
        </div>
     </div>
    ...
    <script>
      ...
      $('#form1').on('submit', function (event) {
        ...
        web3.eth.getAccounts().then(async function (accounts) {
          var receipt = await contract.methods.newItem(prodname, thisdate).send({ from: accounts[0], gas: 1000000 })
            .then(receipt => {
              var msg = "<h5 style='color: #FFF'><b>Item Added Successfully</b></h5><p style='color: #FFF'>Product ID: " + receipt.events.Added.returnValues[0] + "</p>";
              qr.value = receipt.events.Added.returnValues[0];
              $bottom = "<p style='color: #FFF'> You may print the QR Code if required </p>"
              $("#alertText").html(msg);
              $("#qrious").show();
      ...
      $('#form2').on('submit', function (event) {
        ...
        var today = new Date();
        var thisdate = today.getFullYear() + '-' + (today.getMonth() + 1) + '-' + today.getDate();
        var info = "<br><br><b>Date: " + thisdate + "</b><br>Location: " + prodlocation + "<br>Updated by: " + updator;
        web3.eth.getAccounts().then(async function (accounts) {
